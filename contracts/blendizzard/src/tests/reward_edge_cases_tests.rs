...
fn test_multiple_winners_share_rewards() {
    ...
    blendizzard.select_faction(&p2, &0); // WholeNoodle
    blendizzard.select_faction(&p3, &0); // WholeNoodle
    blendizzard.select_faction(&opponent, &1); // PointyStick
    ...
    let p1_epoch = blendizzard.get_epoch_player(&current_epoch, &p1);
    let p2_epoch = blendizzard.get_epoch_player(&current_epoch, &p2);
    let p3_epoch = blendizzard.get_epoch_player(&current_epoch, &p3);
...
fn test_reward_distribution_sums_to_pool() {
    ...
    for i in 0..10 {
        let reward = blendizzard.claim_epoch_reward(&players.get(i as u32).unwrap(), &0);
...
fn test_single_player_gets_all_rewards() {
    ...
    blendizzard.select_faction(&winner, &0); // WholeNoodle
    blendizzard.select_faction(&loser, &1); // PointyStick
    ...
    let winner_reward = if winning_faction == Some(0) {
        blendizzard.claim_epoch_reward(&winner, &0)
    } else {
        blendizzard.claim_epoch_reward(&loser, &0)
    };
