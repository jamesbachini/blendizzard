...
fn test_epoch_cycle_before_duration_fails() {
    ...
    let (_game_contract, _vault_addr, _mock_vault, blendizzard) = setup_epoch_test_env(&env);
...
fn test_epoch_cycle_with_tie_in_standings() {
    let env = setup_test_env();
    let (game_contract, _vault_addr, mock_vault, blendizzard) = setup_epoch_test_env(&env);
...
fn test_epoch_cycle_swap_failure_handling() {
    ...
    let cycle_result = blendizzard.try_cycle_epoch();
